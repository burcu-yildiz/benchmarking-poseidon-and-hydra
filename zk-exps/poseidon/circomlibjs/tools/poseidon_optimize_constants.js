...
const N_ROUNDS_F = 8;
//updated rounds for pallas prime
const N_ROUNDS_P = [56, 56, 56, 56, 57, 57, 57, 57, 57, 57, 57, 57, 57, 57, 57, 57];
...
function vec_mul_matrix(Fr, v, A) {
    checkSquare(A);
    //console.log(v.length, A.length, A);
    if (v.length != A.length) throw new Error("Invalid vec mul sizes");
    const res = [];
    for (let j = 0; j < v.length; j++) {
        res[j] = Fr.zero;
        for (let i = 0; i < A.length; i++) {
            //console.log("v is", v, v[i], A[i][j], res[j]);
...
function convertConstants(Fr, t, C, M) {
    ...
    for (let r = nRoundsF / 2 + nRoundsP; r < nRoundsF + nRoundsP - 1; r++) {
        const cr = C.slice((r + 1) * t, (r + 1) * t + t);
        //console.log("203", (r + 1) * t, (r + 1) * t + t, r, cr, Minv);
        const crt = vec_mul_matrix(Fr, cr, Minv);
        for (let k = 0; k < t; k++) res.push(crt[k]);
    }
...
async function run() {
    // Prime bn128
    //overwritten: pallas
    const Fr = new ZqField(Scalar.fromString("28948022309329048855892746252171976963363056481941560715954676764349967630337"));
