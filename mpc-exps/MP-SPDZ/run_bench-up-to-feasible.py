...
time_limit = 12 #ethereum block time in s
...
result_file = "results-up-to-feasible.csv"
...
def run_poseidons(time_limit, update_func):
    for p in ["poseidon-2-0-hash", "poseidon-2-0-encrypt"]:
        if p == "poseidon-2-0-hash":
            pt_len = 4
            t = 8
        else:
            pt_len = 8
        ...
        parties = 2
        while True:
            print(f"Running {p} with pt_len {pt_len} out_lne {t} rate 2 parties {parties}", flush=True)
            vals = {}
            ret = compile(p, 2, t, pt_len)

            squares, triples, inverses, rounds = ret
            vals["program"] = p
            vals["using squares"] = None
            ...
            vs = run(p, parties, squares, triples, inverses, rounds, RUNS, pt_len, t,2, pt_len, t, pos=True)
            ...
            if vals["party 0 runtime"] > time_limit:
                break

            t, pt_len, parties = update_func(t, pt_len, parties, p)


def run_hydras(time_limit, update_func):
    for p in ["hydra-hash", "hydra-encrypt"]:
        if p == "hydra-hash":
            pt_len = 4
            t = 8
        else:
        ...
        while True:
            ...
            write_hosts(parties)
            print(time.now(), flush=True)
            vs = run(p, parties, squares, triples, inverses, rounds, RUNS, 4, t)#,t, shared_ks, use_squares) #CHANGE
...
def update_length(t, pt_len, parties, p):
    if "poseidon" in p:
        t += 8
    elif "hydra" in p:
        t *= 2
    ...
    if "encrypt" in p:
        pt_len = t
    return t, pt_len, parties

def update_parties(t, pt_len, parties, p):
    return t, pt_len, parties+1
...
def main(runs):
    ...
    run_poseidons(time_limit, update_length)
    run_hydras(time_limit, update_length)
    print("Variable number of parties")
    run_poseidons(time_limit, update_parties)
    run_hydras(time_limit, update_parties)
